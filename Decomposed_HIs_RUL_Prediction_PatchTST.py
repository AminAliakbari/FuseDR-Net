...
from ModelsModify.PatchTST import PatchTSTNetModel
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description="PatchTST RUL Prediction")
    parser.add_argument('--model', default='PatchTST', help='Model name.')
    ...
    parser.add_argument('--stride', type=int, default=2, help='stride for patch_embedding')
...
def run_single_experiment(exp_count, training_dataset, dataloaders, time_varying_known_reals, save_dir, args):
    ...
    model = PatchTSTNetModel.from_dataset(
        training_dataset,
    ...
    best_model = PatchTSTNetModel.load_from_checkpoint(trainer.checkpoint_callback.best_model_path)
