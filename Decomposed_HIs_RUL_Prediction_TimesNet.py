...
from ModelsModify.TimesNet import TimesNetModel
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description="TimesNet RUL Prediction")
    parser.add_argument('--model', default='TimesNet', help='Model name.')
...
def run_single_experiment(exp_count, training_dataset, dataloaders, time_varying_known_reals, save_dir, args):
    ...
    model = TimesNetModel.from_dataset(
        training_dataset,
    ...
    best_model = TimesNetModel.load_from_checkpoint(trainer.checkpoint_callback.best_model_path)
