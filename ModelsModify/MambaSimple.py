...
class MambaSimpleNetModel(BaseModel):
    ...
    # Modified for lithium battery prediction
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:

        x_enc = x["encoder_cont"][:,:,:-1]
        # Output
        prediction = self.network(x_enc, x_mark_enc=None)
        # Rescale output, rescale predictions into target space
        prediction = self.transform_output(prediction, target_scale=x["target_scale"])

        # Return a dictionary containing the output results (prediction)
...
if __name__=='__main__':
    ...
    model=MambaSimple(seq_len=L, enc_in=C, dec_in=C, label_len = label_len, pred_len=pred_len, c_out=1)              # pred_len is constrained
